defmodule Maze  do
  @moduledoc """
  Initializing, building and solving mazes.
  """
  ...
  def initialize(rows \\ @rows ,
    ...
    start_position = pos( Enum.take_random(1..columns,1)
                     |> List.first,
                     Enum.take_random(1..rows,1)
                     |> List.first  )
    room_positions =
  ...
  def go_back_to_previous_visited_room(maze)  do
    maze = if (length(maze.visited_positions) >= 1) do
      Map.update(maze, :visited_positions ,nil,  fn vp ->
        vp |> List.delete(List.first(vp))
      end)
    else
      nil
    end
  ...
  def next_position(maze, direction, position) do
    next_position = case direction do
      :left ->
        if (position.x - 1 >= 1 ),
          do: pos(position.x - 1, position.y ),
        else: nil
      :right ->
        if (position.x + 1 <= maze.columns ),
          do: pos(position.x + 1, position.y),
        else: nil
      :down ->
        if (position.y + 1 <= maze.rows), do:
          pos(position.x, position.y + 1),
        else: nil
      :up ->
        if (position.y - 1 >= 1),
        do: pos(position.x, position.y - 1),
        else: nil
      _  ->
        nil
    end
  ...
  def update_maze_with_built_room(maze, room) do
    ...
    Map.update(maze, :rooms, nil, fn rooms ->
      List.replace_at(rooms, room_index, room)
    end )
  end
  ...
  def solve(maze) do
    with  false  <- current_room(maze).position == maze.goal_position do
      ...
      current_room_built =
        Map.update(current_room(maze), :used_exits, nil,fn ue ->
         [ next_direction | ue ]
        end )
        maze
        |> update_maze_with_built_room(current_room_built)
